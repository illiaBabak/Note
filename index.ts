...
function getTargetElement<T extends HTMLElement>(
  ...
  const searchedElement = [...tagsList].find((el) =>
    [...el.classList].includes(className)
  );
...
function createAndShowModal(
  titleModal: string,
  titleResButton: string,
  key?: string
) {
  if (container.getElementsByClassName("overlay")[0]) return;

  const parsedNotes: unknown = localStorage.getItem("notes")
...
addNoteButton.addEventListener("click", () =>
  createAndShowModal("Add new note", "Add note")
);
...
function addCardInfoToLocalStorage() {
  ...
  const parsedNotes: unknown = notesFromStorage
    ? JSON.parse(notesFromStorage)
    : [];
  ...
  const removeButton = getTargetElement(
    "remove-modal-button",
...
function createCard(key: string) {
  ...
  editButton.addEventListener("click", () =>
    createAndShowModal("Edit note", "Edit", key)
  );
...
function editCard(key: string) {
  ...
  if (title?.value && description?.value) {
    const updatedNotes = notes?.map((note) =>
      note.key === key
        ? { ...note, title: title.value, description: description.value }
        : note
    );
