...
function isPossibleNote(data: unknown): data is PossibleNote {
...
function createAndShowModal(titleModal: string, titleResButton: string, key?: string) {
  if (container.getElementsByClassName('overlay')[0]) return;

  const parsedNotes: unknown = localStorage.getItem('notes') ? JSON.parse(localStorage.getItem('notes') ?? '') : [];
  ...
  if (titleResButton === 'Add note') addNoteButton.addEventListener('click', addCardInfoToLocalStorage);
  if (titleResButton === 'Edit') addNoteButton.addEventListener('click', () => editCard(key ?? ''));
...
function addCardInfoToLocalStorage() {
  ...
  const parsedNotes: unknown = notesFromStorage ? JSON.parse(notesFromStorage) : [];
...
function createCard(key: string) {
...
function removeCard(key: string) {
  const parsedNotes: unknown = JSON.parse(localStorage.getItem('notes') ?? '');
  const notes = isNotesArray(parsedNotes) ? parsedNotes : null;
  const updatedNotes = notes?.filter((el) => el.key !== key);

  localStorage.setItem('notes', JSON.stringify(updatedNotes));
  if (updatedNotes) addCards(updatedNotes);
}
...
function editCard(key: string) {
  ...
  if (title?.value && description?.value) {
    const updatedNotes = notes?.map((note) =>
      note.key === key ? { ...note, title: title.value, description: description.value } : note
    );
